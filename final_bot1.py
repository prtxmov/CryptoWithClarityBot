#!/usr/bin/env python3
# final_bot_admin.py — webhook mode with auto setWebhook on startup
...
def add_user(uid, username, referred_by=None):
    ...
    qdb("INSERT OR IGNORE INTO users (user_id, username, referred_by, referral_id) VALUES (?,?,?,?)",
        (uid, username, referred_by, refid))
...
def is_subscribed(uid):
    ...
    try: return datetime.strptime(r[3], "%Y-%m-%d %H:%M:%S") > datetime.utcnow()
    except: return False
...
def send_main_menu(chat_id):
    ...
    try:
        bot.send_message(chat_id=chat_id, text="🚀 Welcome! Choose an option:", reply_markup=InlineKeyboardMarkup(kb))
    except Exception:
        logger.exception("send menu")
...
def generate_payment_link(user_id, plan_key):
    ...
    payload = {
      "price_amount": amount, "price_currency":"usd", "order_id":order_id,
    ...
    try:
        ...
        return (invoice_url, None) if invoice_url else (None, d.get("error","unknown"))
    except Exception as e:
        logger.exception("create invoice"); return None, str(e)
    ...
    if "callback_query" in data:
        ...
        if payload=="warroom":
            if is_subscribed(cid):
                bot.send_message(chat_id=cid, text="🎟️ You are already subscribed. Use Main Menu to extend.",
        ...
        if payload.startswith("sub_"):
            ...
            bot.send_message(chat_id=cid, text=f"Pay here:\n{link}" if link else f"Error: {err}")
            return jsonify({"ok":True})
        if payload=="earn":
            if is_subscribed(cid):
...
def ipn():
    ...
    logger.info("IPN: headers=%s body=%s", dict(hdr), json.dumps(body))
    ...
    if str(status).lower() in ("finished","paid","successful") and user_id_q:
        try:
            ...
            bot.send_message(chat_id=uid,
                text=f"💥 Hey — upcoming billionaire! Your {label} membership is active.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("➡️ Join Now", url=WARROOM_LINK)]]))
        except Exception: logger.exception("activate ipn")
        return jsonify({"ok":True})
...
def success():
    ...
    if uid:
        bot.send_message(chat_id=uid,
            text="❌ Payment cancelled or timed out. Use Main Menu to try again.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]]))
...
def cancel():
    ...
    if uid:
        bot.send_message(chat_id=uid,
            text="❌ Payment cancelled. Use Main Menu to try again.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]]))
...
def admin_login_page(msg=""):
    ...
    <form method="post" action="/admin">Username: <input name="username"><br>Password: <input type="password" name="password"><br><button>Login</button></form>
    </body></html>
    """
...
def ensure_webhook():
    ...
    try:
        r = requests.post(api, data={"url": url, "drop_pending_updates": "false"}, timeout=10)
        logger.info("setWebhook -> %s %s", r.status_code, r.text)
    except Exception: logger.exception("setWebhook failed")

@app.post("/set_webhook")
def set_webhook_route():
    ensure_webhook()
    return jsonify({"ok": True, "message": "setWebhook called"})

# ---- Startup ----
if __name__ == "__main__":
    init_db()
    ensure_webhook()  # <-- auto-register webhook on startup
